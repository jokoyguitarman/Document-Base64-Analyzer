...
def start_worker():
    ...
    queue_names = os.getenv('CELERY_QUEUES', 'default,page_processing,document_orchestration,batch_processing,audio_generation')
    ...
    worker_args = [
        ...
        '--prefetch-multiplier=4',  # Prefetch more tasks for efficiency
        '--max-tasks-per-child=100',  # Restart workers after 100 tasks to prevent memory leaks
        '--time-limit=1800',  # 30 minute hard timeout
        '--soft-time-limit=1680',  # 28 minute soft timeout
